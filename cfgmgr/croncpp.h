...
// New implementation not found in original code //

template <size_t N>
size_t previous_set_bit(const std::bitset<N>& bits, int start) {
...
void subtract_from_field(std::tm& date, cron_field field, int amount) {
    switch (field) {
        case cron_field::minute:
            date.tm_min -= amount;
            break;
        case cron_field::hour:
            date.tm_hour -= amount;
            break;
        case cron_field::mday:
            date.tm_mday -= amount;
            break;
        case cron_field::mon:
            date.tm_mon -= amount;
    ...
    // Normalize the date to correct underflows
    mktime(&date);
}

size_t cron_previous(std::bitset<60> const& fieldBits, std::tm& date, cron_field field, int start) {
    size_t lastValid = previous_set_bit(fieldBits, start);
    if (lastValid != INVALID_INDEX) {
        switch (field) {
            case cron_field::minute:
                date.tm_min = lastValid;
                break;
            case cron_field::hour:
                date.tm_hour = lastValid;
                break;
            case cron_field::mday:
                date.tm_mday = lastValid;
                break;
            case cron_field::mon:
                date.tm_mon = lastValid - 1;  // month in tm is zero-indexed
                break;
            case cron_field::year:
                date.tm_year = lastValid - 1900;  // tm_year is the number of years since 1900
                break;
            default:
                break;
        }
...
// End of new implementation not found in original code //
