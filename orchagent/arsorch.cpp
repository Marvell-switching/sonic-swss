...
#define ARS_LAG_STAT_COUNTER_FLEX_COUNTER_GROUP "ARS_LAG_STAT_COUNTER"
#define ARS_NHG_STAT_COUNTER_FLEX_COUNTER_GROUP "ARS_NEXTHOP_GROUP_STAT_COUNTER"
#define ARS_STAT_FLEX_COUNTER_POLLING_INTERVAL_MS 10000



const vector<sai_lag_attr_t> lag_counter_stat_ids = {

    SAI_LAG_ATTR_ARS_PACKET_DROPS,
    SAI_LAG_ATTR_ARS_PORT_REASSIGNMENTS
};

const vector<sai_next_hop_group_attr_t> nhg_counter_stat_ids = {
...
#define ARS_FIELD_NAME_QUANTIZATION_BANDS_LIST "quantization_bands_list"
...
ArsOrch::ArsOrch(DBConnector *config_db, DBConnector *appDb, DBConnector *stateDb, vector<table_name_with_pri_t> &tableNames, VRFOrch *vrfOrch) :
        Orch(db, tableNames),
        m_vrfOrch(vrfOrch),
        ...
        m_lag_counter_manager(ARS_LAG_STAT_COUNTER_FLEX_COUNTER_GROUP, StatsMode::READ, ARS_STAT_FLEX_COUNTER_POLLING_INTERVAL_MS, false)
        m_nhg_counter_manager(ARS_NHG_STAT_COUNTER_FLEX_COUNTER_GROUP, StatsMode::READ, ARS_STAT_FLEX_COUNTER_POLLING_INTERVAL_MS, false),
    ...
    switch(type) {
        case SUBJECT_TYPE_PORT_OPER_STATE_CHANGE:
        {
            ...
            if (is_found)
            {
                ...
                {
                    if (!gPortsOrch->setPortArsEnable(update->port, is_found))
                    {
        ...
        {
            /* configure port and lag ARS enable and alternative path memebers */
            PortUpdate *update = reinterpret_cast<PortUpdate *>(cntx);
            if (!update->add)
            {
                if (update->port.m_lag_id && m_arsLags.find(lag.m_alias) != m_arsLags.end())
                {
            ...
            if (is_found)
            {
                ...
                {
                    if ((update->operStatus == SAI_PORT_OPER_STATUS_UP) && !gPortsOrch->setPortArsEnable(update->port))
                    {
                        SWSS_LOG_ERROR("Failed to set ars load scaling factor for port %s", update->port.m_alias.c_str());
                    }
                }
            }

            /* check if this is lag or member of lag */
            if (update->port.m_lag_id)
            {
                Port lag;
                if (gPortsOrch->getPort(update->port.m_lag_id, lag))
                {
                    ...
                    if (isSetImplemented(SAI_OBJECT_TYPE_LAG, SAI_LAG_ATTR_ARS_OBJECT_ID))
                    {
                        ...
                        if (do_action)
                        {
                            ...
                            if (status != SAI_STATUS_SUCCESS)
                            {
                                SWSS_LOG_ERROR("Failed to set pvid %u to lag: %s", attr.value.u32, update->port.m_alias.c_str());
                            }
                            addLagToFlexCounter(update->port.m_lag_id, update->port.m_alias);
                        }
                    }
                    /* check if this is lag member */
                    if (update->port->m_lag_member_id)
                    {
                        if ((lag_object != m_arsLags.end()) && isSetImplemented(SAI_OBJECT_TYPE_PORT, SAI_PORT_ATTR_ARS_ENABLE))
                        {
                            bool enable = (lag_object.second->altPathPorts.find(update->port->m_alias)) ? true : false;
                            gPortsOrch->setPortArsAltPath(update->port->m_alias, enable);
                        }
                    }
                }
            }
        ...
        case SUBJECT_TYPE_NEXTHOP_CHANGE:
        {
            ...
            std::string vrf_name = m_vrfOrch->getVRFname(update->vrf_id);
            ...
            auto ars_object = nhg_table->second.find(prefix);
            ...
            if (nhg_sa_id == SAI_NULL_OBJECT_ID)
            {
                break;
            }
            if (update->nexthopGroup.getSize() > 1)
            {
                if (gRouteOrch->updateNexthopGroupArsState(nhg_sai_id, ars_object->second.m_ars_object_id))
                {
                    addNhgToFlexCounter(nhg_sai_id, ip_prefix, vrf_name);
                }
                gRouteOrch->updateNexthopArsState(update->nexthopGroup, ars_object->second.altPathNexthops);
            }
            break;
        }
        default:
...
bool ArsOrch::delArsObject(ArsObjectEntry &object)
{
    ...
    if (status != SAI_STATUS_SUCCESS)
    {
        SWSS_LOG_ERROR("Failed to remove ars %s (oid %" PRIx64 ")",
                        object->name.c_str(), object->ars_object_id, status);
        task_process_status handle_status = handleSaiSetStatus(SAI_API_ARS, status);
        if (handle_status != task_success)
        {
            return parseHandleSaiStatusFailure(handle_status);
...
void ArsOrch::doTaskArsQuantizationBands(Consumer &consumer)
{
    ...
    while (it != consumer.m_toSync.end())
    {
        ...
        if (op == SET_COMMAND)
        {
            ...
            for (auto i : kfvFieldsValues(t))
            {
                if (fvField(i) == ARS_FIELD_NAME_MIN_VALUE)
                {
                ...
                else if (fvField(i) == ARS_FIELD_NAME_MAX_VALUE)
                {
                ...
                FieldValueTuple value("ARS_QUANTIZATION_BANDS_" + std::to_string(band_index) + "_" + fvField(i), fvValue(i));
                fvVector.push_back(value);
            }
        ...
        else if (op == DEL_COMMAND)
        {
            ...
            for (auto i = 0; i < 8; i ++)
            {
                m_arsProfileStateTable.hdel(ars_profile_name, "ARS_QUANTIZATION_BANDS_" + std::to_string(i) + "_" + ARS_FIELD_NAME_MIN_VALUE);
                m_arsProfileStateTable.hdel(ars_profile_name, "ARS_QUANTIZATION_BANDS_" + std::to_string(i) + "_" + ARS_FIELD_NAME_MAX_VALUE);
            }
        }
...
bool ArsOrch::doTaskArsNexthopGroup(const KeyOpFieldsValuesTuple & t)
{
    ...
    while (it != consumer.m_toSync.end())
    {
        ...
        if (op == SET_COMMAND)
        {
            ...
            {
                is_new_entry = true;
                ArsNexthopGroupPrefixes nhg_table;
                ArsObjectEntry arsObjectEntry;
                arsObjectEntry.profile_name = ars_profile_name;
                nhg_table[ip_prefix] = arsObjectEntry;
                auto temp = nhg_table.find(ip_prefix);
                m_arsNexthopGroupPrefixes[vrf_name] = temp;
                table = m_arsNexthopGroupPrefixes.find(vrf_name);
            }

            arsObject_entry = table.find(ip_prefix);

            if (arsObject_entry == table.end())
            {
                is_new_entry = true;
                ArsObjectEntry arsObjectEntry;
                arsObject_entry = table.find(ip_prefix);
                table[ip_prefix] = arsObjectEntry;
            }
            ...
            for (auto i : kfvFieldsValues(t))
            {
                if (fvField(i) == ARS_FIELD_NAME_PROFILE_NAME)
                {
                    ...
                    arsObject_entry->second.profile_name = ars_profile_name;
                }
                else if (fvField(i) == ARS_FIELD_NAME_MAX_FLOWS)
                {
                    arsObject_entry->second.max_flows = stoi(fvValue(i));
                ...
                {
                    arsObject_entry->second.flowlet_idle_time = stoi(fvValue(i));
                ...
                {
                    arsObject_entry->second.path_quality.primary_threshold = stoi(fvValue(i));
                ...
                {
                    arsObject_entry->second.path_quality.alt_threshold = stoi(fvValue(i));
                ...
                if (fvField(i) != ARS_FIELD_NAME_ALT_PATH_MEMBERS)
                {
                    FieldValueTuple value(fvField(i), fvValue(i));
                    fvVector.push_back(value);
                }
            ...
            SWSS_LOG_NOTICE("Ars entry added for profile %s, prefix %s, NHs", ars_profile_name.c_str(), ip_prefix.to_string().c_str(), nhg.to_string().c_str());
            ...
            if (nhg.getSize() > 0)
            {
                ...
                if (nhg_sai_id)
                {
                    if (!isSetImplemented(SAI_OBJECT_TYPE_NEXT_HOP_GROUP, SAI_NEXT_HOP_GROUP_ATTR_ARS_OBJECT_ID))
                    {
                        ...
                        if (gRouteOrch->reconfigureNexthopGroupWithArsState(nhg, nhg_sai_id, arsObject_entry->second.m_ars_object_id))
                        {
                            addNhgToFlexCounter(nhg_sai_id, ip_prefix, vrf_name);
                        }
                    }
                    else if (gRouteOrch->updateNexthopGroupArsState(nhg_sai_id, arsObject_entry->second.m_ars_object_id))
                    {
                        addNhgToFlexCounter(nhg_sai_id, ip_prefix, vrf_name);
                    }
                }
                /* enable ARS alternative path over all nexthops in group */
                gRouteOrch->updateNexthopArsState(nhg, arsObject_entry->second.altPathNexthops);
            }
        }
        else if (op == DEL_COMMAND)
        {
            ...
            {
                arsObject_entry = table.find(ip_prefix);
                if (arsObject->second.m_ars_object_id)
                {
                    /* handle existing nexthop group */
                    if (nhg.getSize() > 0)
                    {
                        /* Disable ARS over all nexthops in group */
                        gRouteOrch->updateNexthopArsState(nhg, {});
                    ...
                    delArsObject(arsObject_entry->second.m_ars_object_id);
                }

                arsObject_entry->second->prefixes.erase(ip_prefix.to_string());
                ...
                m_arsNhgStateTable.erase(vrf_name + "_"  + ip_prefix.to_string());

                auto arsProfile_entry = m_arsProfiles.find(arsObject_entry->second.profile_name);
                if (arsProfile_entry == m_arsProfiles.end())
                {
                    SWSS_LOG_ERROR("Received ARS NHG Vrf %s, Prefix %s for non-existent profile %s", vrf_name.c_str(), ip_prefix.to_string().c_str(), arsObject_entry->second.profile_name.c_str());
                }
                else
                {
                    arsProfile_entry->second.ref_count--;
...
bool ArsOrch::doTaskArsLag(Consumer &consumer)
{
    ...
    while (it != consumer.m_toSync.end())
    {
        ...
        SWSS_LOG_NOTICE("ARS Lag Op %s, Profile: %s, name %s", op.c_str(), ars_profile_name.c_str(), if_name.c_str());

        auto table = m_arsLags.find(if_name);

        ArsObjectEntry *arsObject_entry = nullptr;
        if (op == SET_COMMAND)
        {
            if (table == m_arsLags.end())
            {
                is_new_entry = true;
                ArsObjectEntry arsObjectEntry;
                arsObjectEntry.profile_name = ars_profile_name;
            ...
            arsObject_entry = table.find(if_name);
            ...
            string ifnames = "";
            for (auto ifname : arsObject_entry->second.altPathPorts)
            {
                ifnames += ifname + " ";
            }
            FieldValueTuple tmp("alt_ports", ifnames);
            vector<FieldValueTuple> fvVector;
            fvVector.push_back(tmp);
...
{
    return string(ARS_LAG_STAT_COUNTER_FLEX_COUNTER_GROUP) + ":" + key;
}

string ArsOrch::getNhgFlexCounterTableKey(string key)
{
    return string(ARS_NHG_STAT_COUNTER_FLEX_COUNTER_GROUP) + ":" + key;
}
...
std::unordered_set<std::string> ArsOrch::generateLagCounterStats()
{
    ...
    {
        if (isGetImplemented(SAI_OBJECT_TYPE_LAG, it))
        {
            counter_stats.emplace(sai_serialize_attr_id(it));
        }
    }
...
std::unordered_set<std::string> ArsOrch::generateNhgCounterStats()
{
    ...
    {
        if (isGetImplemented(SAI_OBJECT_TYPE_NEXT_HOP_GROUP, it))
        {
            counter_stats.emplace(sai_serialize_attr_id(it));
        }
    }
...
void ArsOrch::removeLagFromFlexCounter(sai_object_id_t id, const string &name)
{
    ...
    {
        m_lag_counter_manager.erase(update_iter);
    }
...
void ArsOrch::removeNhgFromFlexCounter(sai_object_id_t id, const IpPrefix &prefix, const string &vrf)
{
    ...
    /* remove it from COUNTERS_DB maps */
    m_counter_table->hdel("", prefix.to_string() + ":" + vrf);
    m_vidToRidTable->hdel("", counter_oid_str);

    SWSS_LOG_DEBUG("Unregistered lag %s from Flex counter", name.c_str());
}
...
void ArsOrch::doTask(SelectableTimer &timer)
{
    ...
    if (m_LagFlexCounterUpdTimer == timer)
    {
        for (auto it = m_pendingLagAddToFlexCntr.begin(); it != m_pendingLagAddToFlexCntr.end(); )
        {
            ...
            if (!gTraditionalFlexCounter || m_vidToRidTable->hget("", id, value))
            {
                ...
                m_counter_table->set("", lagNameVector);
    ...
    if (m_NhgFlexCounterUpdTimer == timer)
    {
        for (auto it = m_pendingNhgAddToFlexCntr.begin(); it != m_pendingNhgAddToFlexCntr.end(); )
        {
            ...
            if (!gTraditionalFlexCounter || m_vidToRidTable->hget("", id, value))
            {
                ...
                m_counter_table->set("", nhgNameVector);
                m_counter_db->hset(COUNTERS_ARS_NEXTHOP_GROUP_NAME_MAP, it->second.c_str(), id.c_str());

                auto nhg_counter_stats = generateLagCounterStats();
                m_lag_counter_manager.setCounterIdList(it->first, CounterType::ARS_NEXTHOP_GROUP, nhg_counter_stats, gSwitchId);
                SWSS_LOG_DEBUG("inserted %s to flex counter", it->second.c_str());
